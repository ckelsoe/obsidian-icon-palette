...
describe('ColorUtils.toRgb', () => {
	it('returns the grey fallback for an unrecognised colour', () => {
		expect(ColorUtils.toRgb('definitely-not-a-colour')).toBe(
			'rgb(128, 128, 128)',
		);
	});
});
...
describe('ColorUtils.toRgbObject', () => {
	it('parses the grey fallback into an RGB object', () => {
		expect(ColorUtils.toRgbObject('definitely-not-a-colour')).toEqual({
			r: 128,
			g: 128,
			b: 128,
		});
...
describe('ColorUtils.toHslArray', () => {
	it('converts the grey fallback to HSL grey', () => {
		expect(ColorUtils.toHslArray('definitely-not-a-colour')).toEqual([
			0, 0, 50,
		]);
	});
});
...
describe('ColorUtils.mixToRgb', () => {
	...
	it('honours unequal percentages', () => {
		expect(
			ColorUtilsInternal.mixToRgb(
				'color-mix(in srgb, rgb(255, 0, 0) 75%, rgb(0, 0, 0) 25%)',
			),
		).toBe('rgb(191, 0, 0)');
	});

	it('returns black when the string does not match the colour-mix shape', () => {
		expect(ColorUtilsInternal.mixToRgb('not-a-color-mix')).toBe(
			'rgb(0, 0, 0)',
		);
	});
});
