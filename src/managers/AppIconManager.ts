...
import type { AppItemId } from 'src/types.js';
...
const SVG_INFO = {
	attr: { 'aria-hidden': false, width: 12, height: 12, viewBox: '0 0 12 12' },
};
const MINIMIZE_RECT = {
	attr: { fill: 'currentColor', width: 10, height: 1, x: 1, y: 6 },
};
const MAXIMIZE_RECT = {
	attr: {
		width: 9,
...
const UNMAXIMIZE_PATH_1 = {
	attr: { d: 'M1.5 3.5H8.5V10.5H1.5V3.5Z', stroke: 'currentColor' },
};
const UNMAXIMIZE_PATH_2 = {
	attr: { d: 'M4 2H10V8H9V9H11V1H3V3H4V2Z', fill: 'currentColor' },
};
const CLOSE_PATH_1 = {
	attr: {
		fill: 'currentColor',
...
export default class AppIconManager extends IconManager {
	...
	refreshIcons(unloading?: boolean): void {
		...
		if (Platform.isMobile) {
			if (
				...
				this.pinEls.some((el) => !activeDocument.contains(el))
			) {
				for (const el of this.pinEls)
		...
		if (
			...
			this.sidebarLeftEls.some((el) => !activeDocument.contains(el))
		) {
			for (const el of this.sidebarLeftEls)
				this.stopEventListener(el, 'contextmenu');
			this.sidebarLeftEls = fishAll(
		...
		for (const sidebarLeftEl of this.sidebarLeftEls) {
			const iconEl = sidebarLeftEl.hasClass('clickable-icon')
				? sidebarLeftEl
				: sidebarLeftEl.find(':scope > .clickable-icon');
			if (iconEl) {
		...
		if (this.minimizeEl) {
			...
			if (item.icon) {
				...
				this.minimizeEl.removeClass('icon-palette-icon');
				const rectEl = this.minimizeEl
					.createSvg('svg', SVG_INFO)
					.createSvg('rect', MINIMIZE_RECT);
				if (item.color)
					rectEl.setAttr('fill', ColorUtils.toRgb(item.color));
			}
			if (this.plugin.settings.showMenuActions) {
				this.setEventListener(
					this.minimizeEl,
		...
		if (this.closeEl) {
			...
			if (item.icon) {
				...
				this.closeEl.empty();
				this.closeEl.removeClass('icon-palette-icon');
	...
	private refreshMaximizeIcon(unloading?: boolean): void {
		...
		if (this.maximizeEl) {
			const item = this.plugin.getAppItem(
			...
			if (item.icon) {
				...
				this.maximizeEl.removeClass('icon-palette-icon');
				...
				if (isMaximized) {
					...
					if (item.color)
						rectEl.setAttr('stroke', ColorUtils.toRgb(item.color));
				}
			}
			if (this.plugin.settings.showMenuActions) {
				this.setEventListener(
	...
	private onContextMenu(appItemId: AppItemId, event: MouseEvent): void {
		...
		const menu =
			appItemId.startsWith('sidebar') && !Platform.isPhone
