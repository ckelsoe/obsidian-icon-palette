...
type LibraryId = 'simple' | 'devicon';

type IconDimensions =
	0 | [left: number, top: number, width: number, height: number];
type SerializedIconDefinition = [
	id: string,
	name: string,
	body: string,
...
function decodeIconData(): SerializedIconDefinition[] {
	...
	const compressed = Uint8Array.from(binary, (char: string) =>
		char.charCodeAt(0),
	);
	return JSON.parse(
		strFromU8(inflateSync(compressed)),
	) as SerializedIconDefinition[];
}
...
function getLibraryIcons(): IconDefinition[] {
	cachedLibraryIcons ??= decodeIconData().map(
		([id, name, body, dimensions]) => ({
			id,
			name,
...
export function registerIconLibraries(ids?: Iterable<string>): void {
	...
	for (const icon of getLibraryIcons()) {
		...
		if (registeredAllLibraryIcons || registeredLibraryIconIds.has(icon.id))
			continue;
 ...
 */
export function populateLibraryIcons(
