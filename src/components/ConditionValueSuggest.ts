...
export default class ConditionValueSuggest extends AbstractInputSuggest<ConditionSuggestion> {
		...
		switch (this.page) {
			case 'file': {
				switch (this.condition.source) {
					case 'name':
						return SUGGEST_FILE_NAMES;
					case 'filename':
						return SUGGEST_FILE_FILENAMES;
					case 'extension':
						return SUGGEST_FILE_EXTENSIONS;
					case 'tree':
						return SUGGEST_FOLDER_PATHS;
					case 'path':
						return SUGGEST_FILE_PATHS;
				}
				break;
			}
			case 'folder': {
	...
	protected getSuggestions(
		query: string,
	): ConditionSuggestion[] | Promise<ConditionSuggestion[]> {
		...
		switch (this.getSuggestionType()) {
			...
			case SUGGEST_FILE_EXTENSIONS: {
				...
				const extensions = new Set(
					tFiles.map((tFile) => tFile.extension),
				);
