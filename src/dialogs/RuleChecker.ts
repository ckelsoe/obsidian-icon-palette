...
export default class RuleChecker extends Modal {
	...
	onOpen(): void {
			...
			case 'file': {
				this.setTitle(
					this.matches.length === 1
						...
						: STRINGS.ruleChecker.filesMatch.replace(
								'{#}',
			...
			case 'folder': {
				this.setTitle(
					this.matches.length === 1
						...
						: STRINGS.ruleChecker.foldersMatch.replace(
								'{#}',
								this.matches.length.toString(),
							),
				);
		...
		const defaultIcon =
			this.page === 'folder' ? 'lucide-folder' : 'lucide-file';
		for (const match of this.matches) {
			const { tree, basename, extension } = this.plugin.splitFilePath(
				match.id,
			);
			const rule =
				this.plugin.ruleManager?.checkRuling(this.page, match.id) ??
				match;
